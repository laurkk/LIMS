 ...
 <body>
	...
	<main>	
        ...
        <section class="results">	
				<p class="content_title"> Add New Lab Equpiment: </p>	
		...
		<form action="add_equipment.php" method="POST">
			...
			$Prod = $dropDown -> getDropdownOptions("producer","idproducer",
			"name",$conn);
			$EmpName = $dropDown -> getDropdownOptions("w_personal_data","id_personal_data",
			'CONCAT(name," ",surname)',$conn);
		
			?>
			<label for="prodName">Producer:</label>
			<select name="prodName" id="prodName" required>
				<?php echo $Prod ?>
			</select><br><br>
			
			<label for="empName">Responsibility:</label>
			<select name="empName" id="empName" required>
				<?php echo $EmpName ?>
			</select><br><br>
            
            <label for="eqName">Equipment name</lable>
            <input type="text" name="eqName" id="eqName" required><br><br>

        	<label for="prodDate">Production date:</label>
        	<input type="date" name="prodDate" id="prodDate" value = "1980-01-01"/><br><br>
            
            <label for="locali"> Localisation in Lab:</label>
            <input type="number" name="locali" id="locali" min='0'><br><br>
